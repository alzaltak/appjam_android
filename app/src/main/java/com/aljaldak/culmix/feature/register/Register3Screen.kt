...
@Composable
fun Register3Screen(onClick: () -> Unit, viewModel: RegisterViewModel) {
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            StepsProgressBar(
                numberOfSteps = 5, currentStep = 3, modifier = Modifier.padding(vertical = 30.dp)
            )
            Text(
                text = "회원가입",
            ...
            TextField(modifier = Modifier.fillMaxWidth(), value = id.value, onValueChange = {
                id.value = it
                viewModel.account_id = it
            }, colors = TextFieldDefaults.outlinedTextFieldColors(
                backgroundColor = Color(0xFFFFFFFF), focusedBorderColor = Main
            ), placeholder = {
                Text(text = "아이디")
            })

            TextField(modifier = Modifier.fillMaxWidth(), value = password.value, onValueChange = {
                password.value = it
